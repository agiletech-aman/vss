...
@section('title', 'Warehouse Activity')
@section('content')
...
<div class="ew-bc">
    <div class="ew-bc-trail">
        ...
        <a href="{{ route('nms.pages.warehouses') }}">Warehouses</a>
        <i class="ri-arrow-right-s-line"></i>
        <span id="breadName" style="color:var(--text)">Loading…</span>
    </div>
    <a href="{{ route('nms.pages.warehouses') }}" class="ew-back"><i class="ri-arrow-left-line"></i> Back</a>
</div>
...
const NMS_BASE  = @json($nmsBase);
const FRS_BASE  = @json($frsBase);
const SACK_BASE = @json($sackBase);
const IOT_BASE  = @json($iotBase);
const CO2_URL   = @json($co2SummaryUrl);
...
/* ════════════════════════════════
   5. IoT — CO₂ / PH₃
   {{ config('external-apis.iot_base') }}/api/master-alerts
