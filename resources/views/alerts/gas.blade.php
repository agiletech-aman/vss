...
<div class="page-header">
    ...
    <div style="display:flex;align-items:center;gap:10px;">
        <div class="gas-toggle-buttons">
            <a href="{{ route('gas.alerts.index', array_merge(request()->except(['deviceTypeId','page']), ['deviceTypeId' => 30000])) }}"
               class="btn {{ $deviceTypeId == 30000 ? 'btn-success' : 'btn-outline-success' }}">
                <i class="ri-cloud-line me-1"></i> CO₂
            </a>
            <a href="{{ route('gas.alerts.index', array_merge(request()->except(['deviceTypeId','page']), ['deviceTypeId' => 30001])) }}"
...
{{-- KPI Cards --}}
<div class="kpi-row">
    <a href="{{ route('gas.alerts.index', array_merge(request()->except(['alertType','page']), ['deviceTypeId'=>$deviceTypeId])) }}"
       class="kpi-card kpi-blue {{ empty($selectedAlertType) ? 'active' : '' }}">
        <div class="kpi-icon"><i class="ri-bar-chart-box-line"></i></div>
        <div><div class="kpi-label">Total Alerts</div><div class="kpi-value">{{ number_format($kpis['total']) }}</div></div>
    </a>
    <a href="{{ route('gas.alerts.index', array_merge(request()->except(['alertType','page']), ['deviceTypeId'=>$deviceTypeId,'alertType'=>'NORMAL'])) }}"
    ...
    <a href="{{ route('gas.alerts.index', array_merge(request()->except(['alertType','page']), ['deviceTypeId'=>$deviceTypeId,'alertType'=>'SEVERE'])) }}"
    ...
    <a href="{{ route('gas.alerts.index', array_merge(request()->except(['alertType','page']), ['deviceTypeId'=>$deviceTypeId,'alertType'=>'CRITICAL'])) }}"
    ...
    <form method="GET" id="filterForm" class="row g-2 align-items-end">
            ...
            <select id="stateSelect" name="state" class="form-select">
                ...
                @foreach($states as $s)
                    ...
                    <option value="{{ $stateId }}"
                            {{ (string) $selectedState === (string) $stateId ? 'selected' : '' }}>
                        {{ $stateName }}
                    </option>
                @endforeach
            </select>
        ...
        <div class="col-md-2">
            ...
            <select id="locationSelect" name="location" class="form-select">
                ...
                @foreach($locations as $loc)
                    ...
                    @endphp
                    <option value="{{ $locationId }}" {{ (string) $selectedLocation === (string) $locationId ? 'selected' : '' }}>
                        {{ $locationName }}
                    </option>
                @endforeach
            </select>
        ...
        <div class="col-md-2">
            ...
            <select name="alertType" class="form-select">
                ...
                <option value="UNKNOWN"  {{ $selectedAlertType=='UNKNOWN'  ? 'selected' : '' }}>Unknown</option>
            </select>
        </div>
        <div class="col-md-2">
        ...
        <div class="col-md-2 d-flex gap-2 align-items-end">
            ...
            <a href="{{ route('gas.alerts.index', ['deviceTypeId' => $deviceTypeId]) }}"
    ...
    <div class="table-card-header">
        <div class="table-card-title">
            ...
            @if($selectedAlertType)
                <span class="badge {{ $selectedAlertType=='CRITICAL' ? 'badge-critical' : ($selectedAlertType=='SEVERE' ? 'badge-severe' : ($selectedAlertType=='UNKNOWN' ? 'badge-unknown' : 'badge-normal')) }}">
                ...
                <tbody id="alertTableBody">
                    @forelse($alerts as $alert)
                        ...
                        <tr>
                            ...
                            <td><span class="badge {{ $statusClass }}">{{ $deviceStatus !== '' ? $deviceStatus : '-' }}</span></td>
    ...
    <nav>
        <ul class="pagination mb-0">
            <li class="page-item {{ $page<=1 ? 'disabled' : '' }}">
                <a class="page-link" href="{{ $page<=1 ? '#' : route('gas.alerts.index', array_merge(request()->except('page'),['page'=>1])) }}">
                    <i class="ri-skip-back-mini-line"></i> First
                </a>
            </li>
            <li class="page-item {{ $page<=1 ? 'disabled' : '' }}">
                <a class="page-link" href="{{ $page<=1 ? '#' : route('gas.alerts.index', array_merge(request()->except('page'),['page'=>max(1,$page-1)])) }}">
            ...
            @for($i=$start; $i<=$end; $i++)
                <li class="page-item {{ $i==$page ? 'active' : '' }}">
                    <a class="page-link" href="{{ route('gas.alerts.index', array_merge(request()->except('page'),['page'=>$i])) }}">{{ number_format($i) }}</a>
                </li>
            @endfor
            <li class="page-item {{ $page>=$totalPages ? 'disabled' : '' }}">
                <a class="page-link" href="{{ $page>=$totalPages ? '#' : route('gas.alerts.index', array_merge(request()->except('page'),['page'=>min($totalPages,$page+1)])) }}">
                    Next <i class="ri-arrow-right-s-line"></i>
                </a>
            </li>
            <li class="page-item {{ $page>=$totalPages ? 'disabled' : '' }}">
                <a class="page-link" href="{{ $page>=$totalPages ? '#' : route('gas.alerts.index', array_merge(request()->except('page'),['page'=>$totalPages])) }}">
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function loadLocations(stateId, preselectValue = null) {
        ...
        fetch(`{{ route('gas.alerts.locations', ['state' => '__STATE__']) }}`.replace('__STATE__', encodeURIComponent(stateId)))
            .then(res => res.json())
            .then(data => {
...
function exportToServer() {
    ...
    window.location.href = '{{ route("gas.alerts.export") }}?' + params.toString();
}
</script>
@endpush
