        ...
        try {
            ...
            $responses = Http::pool(function ($pool) use ($states, $timeout) {
                foreach ($states as $index => $state) {
                    $stateId = is_array($state)
                        ? ($state['base_id'] ?? $state['id'] ?? $state['name'] ?? null)
                        : $state;

                    if (!is_scalar($stateId) || trim((string) $stateId) === '') {
                        continue;
                    }

                    $pool->as('state_' . $index)
                         ->timeout($timeout)
                         ->withoutVerifying()
                         ->get($this->locationsUrl((string) $stateId));
                }
            });
            ...
            foreach ($responses as $state => $response) {
                if (
                    ...
                    foreach ($locs as $location) {
                        $locationName = is_array($location)
                            ? ($location['name'] ?? $location['locationName'] ?? $location['location'] ?? null)
                            : $location;

                        if (is_scalar($locationName) && trim((string) $locationName) !== '') {
                            $locations[] = trim((string) $locationName);
                        }
                    }
                } else {
