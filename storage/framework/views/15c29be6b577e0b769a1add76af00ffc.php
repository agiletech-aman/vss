<?php $__env->startSection('title', 'Rodent Alerts'); ?>
...
:root {
    ...
    --alert-border: #e2e8f0; --alert-rodent: #7c3aed; --alert-rodent-bg: #f5f3ff;
    --alert-rodent-light: #ede9fe; --alert-radius: 12px;
...
.alert-icon { width:44px; height:44px; background:linear-gradient(135deg,#7c3aed,#a78bfa); border-radius:12px; display:flex; align-items:center; justify-content:center; font-size:20px; color:#fff; box-shadow:0 4px 12px rgba(124,58,237,.3); }
...
.filter-card .form-select:focus, .filter-card .form-control:focus { border-color:var(--alert-rodent); background:#fff; box-shadow:0 0 0 3px rgba(124,58,237,.08); outline:none; }
.filter-actions { display:flex; gap:10px; }
.btn-filter { height:48px; background:linear-gradient(135deg,#7c3aed,#6d28d9); color:#fff; border:none; border-radius:10px; font-size:14px; font-weight:700; padding:0 24px; transition:all .2s; box-shadow:0 4px 12px rgba(124,58,237,.25); cursor:pointer; }
.btn-filter:hover { background:linear-gradient(135deg,#6d28d9,#5b21b6); transform:translateY(-2px); }
.btn-reset { height:48px; background:var(--alert-card); color:var(--alert-muted); border:1.5px solid var(--alert-border); border-radius:10px; font-size:14px; font-weight:600; padding:0 20px; transition:all .2s; display:inline-flex; align-items:center; justify-content:center; text-decoration:none; }
.btn-reset:hover { background:#f8fafc; border-color:#cbd5e1; color:var(--alert-text); }
.info-banner { background:var(--alert-rodent-bg); border:1px solid var(--alert-rodent-light); border-left:4px solid var(--alert-rodent); border-radius:10px; padding:14px 18px; margin-bottom:20px; display:flex; align-items:center; gap:12px; font-size:13px; color:#4c1d95; font-weight:600; }
...
.table-card-title i { color:var(--alert-rodent); }
#rodentTable { width:100%; margin:0; border-collapse:collapse; }
#rodentTable thead th { background:#f9fafb; color:var(--alert-muted); font-size:11px; font-weight:700; text-transform:uppercase; letter-spacing:.5px; padding:16px 20px; border-bottom:1px solid var(--alert-border); white-space:nowrap; text-align:left; cursor:pointer; user-select:none; }
#rodentTable thead th:after { content:' ⇅'; font-size:10px; opacity:.4; }
#rodentTable thead th.sort-asc:after { content:' ↑'; opacity:1; }
#rodentTable thead th.sort-desc:after { content:' ↓'; opacity:1; }
#rodentTable thead th.no-sort { cursor:default; } #rodentTable thead th.no-sort:after { content:''; }
#rodentTable tbody td { padding:16px 20px; color:var(--alert-text); font-size:14px; font-weight:500; border-bottom:1px solid #f3f4f6; vertical-align:middle; }
#rodentTable tbody tr:hover { background:#f9fafb; }
#rodentTable tbody tr:last-child td { border-bottom:none; }
...
.alert-badge { display:inline-flex; align-items:center; gap:6px; padding:6px 14px; background:var(--alert-rodent-bg); color:var(--alert-rodent); border:1px solid var(--alert-rodent-light); border-radius:8px; font-size:12px; font-weight:700; text-transform:uppercase; letter-spacing:.5px; }
...
.preview-img:hover { transform:scale(1.05); border-color:var(--alert-rodent); }
...
.pagination .page-link:hover { background:var(--alert-rodent); color:#fff; border-color:var(--alert-rodent); transform:translateY(-1px); }
.pagination .page-item.active .page-link { background:var(--alert-rodent); border-color:var(--alert-rodent); color:#fff; }
...
<div style="padding:0;">
    ...
    <div class="alert-header">
        <div class="alert-title">
            <div class="alert-icon"><i class="ri-bug-fill"></i></div>
            <div><h4>Rodent Detection Alerts</h4><div class="subtitle">Camera Logs &amp; Activity</div></div>
        </div>
        <a href="<?php echo e(route('dashboard')); ?>" class="alert-back-btn"><i class="ri-arrow-left-line"></i> Back to Dashboard</a>
    </div>
        ...
        <form method="GET" class="row g-3">
            ...
            <div class="col-md-2">
                ...
                <div class="filter-actions">
                    ...
                    <a href="<?php echo e(route('alerts.rodent')); ?>" class="btn-reset"><i class="ri-refresh-line"></i></a>
                </div>
            </div>
        </form>
    </div>
    ...
    <div class="table-card">
        <div class="table-card-header">
            <h5 class="table-card-title"><i class="ri-bug-line"></i> Rodent Alert Logs</h5>
            <div style="display:flex;align-items:center;gap:10px;">
                ...
                <a href="<?php echo e(route('alerts.rodent.export')); ?>?<?php echo e(http_build_query(request()->query())); ?>"
                   style="display:inline-flex;align-items:center;gap:6px;background:linear-gradient(135deg,#7c3aed,#6d28d9);color:#fff;border:none;border-radius:8px;font-size:13px;font-weight:600;height:34px;padding:0 14px;cursor:pointer;box-shadow:0 4px 10px rgba(124,58,237,.25);text-decoration:none;">
        ...
        <div class="table-responsive">
            <table id="rodentTable" class="table align-middle mb-0">
                <thead>
                    <tr>
                ...
                <tbody id="rodentTableBody">
                    <?php $__currentLoopData = $alerts; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $row): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        <tr>
                            ...
                            <td><span class="alert-badge"><i class="ri-bug-fill"></i> Rodent</span></td>
                            <td class="text-center">
                                <?php if(!empty($row['imagePath'])): ?>
                                    <?php $imageUrl = 'https://co2ph3master.ajeevi.in' . $row['imagePath']; ?>
                                    <img src="<?php echo e($imageUrl); ?>" class="preview-img" data-full="<?php echo e($imageUrl); ?>" loading="lazy" onerror="this.style.display='none'" alt="Rodent Alert">
                                <?php else: ?>
        ...
        <?php else: ?>
        <div class="empty-state"><i class="ri-bug-line"></i><p>No rodent alerts found.</p></div>
        <?php endif; ?>
    </div>
...
document.addEventListener('DOMContentLoaded', function () {
    const table = document.getElementById('rodentTable');
...
<?php echo $__env->make('layout.master', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH D:\xampp\htdocs\ATS\vss\resources\views\alerts\rodent.blade.php ENDPATH**/ ?>