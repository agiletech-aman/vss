...
:root{
    ...
    --amber:#f59e0b; --amber-l:#fffbeb; --amber-b:#fde68a;
    --r:12px; --rs:8px;
...
/* ── Top bar ── */
.nms-topbar { display:flex; align-items:center; justify-content:space-between; margin-bottom:22px; flex-wrap:wrap; gap:12px; }
.nms-topbar-left { display:flex; align-items:center; gap:14px; }
.nms-back { display:inline-flex; align-items:center; gap:5px; padding:6px 12px; font-size:12px; font-weight:700; color:var(--text-2); background:#fff; border:1.5px solid var(--border); border-radius:var(--rs); text-decoration:none; transition:all .16s; }
.nms-back:hover { border-color:var(--navy); color:var(--navy); text-decoration:none; }
.nms-page-title { font-size:20px; font-weight:800; color:var(--text); letter-spacing:-.3px; }
.nms-page-sub   { font-size:12px; color:var(--muted); font-weight:500; margin-top:2px; }
...
.nms-card-body { padding:18px 20px; }
  
/* ── Table ── */
.nms-tbl { width:100%; border-collapse:collapse; font-size:12.5px; }
.nms-tbl thead th { background:#f8fafc; color:var(--muted); font-size:9.5px; font-weight:800; text-transform:uppercase; letter-spacing:.08em; padding:10px 14px; border-bottom:1.5px solid var(--border); text-align:left; white-space:nowrap; position:sticky; top:0; z-index:1; }
...
.badge-amb { background:#fffbeb;          color:#92400e;         border-color:#fde68a; }
...
.nms-btn-primary:hover { background:#0f172a; transform:translateY(-1px); }
...
@keyframes nmsFadeUp{ from{opacity:0;transform:translateY(12px);}to{opacity:1;transform:translateY(0);} }
</style>
<?php /**PATH D:\xampp\htdocs\ATS\vss\resources\views\partials\styles.blade.php ENDPATH**/ ?>